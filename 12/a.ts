...
// function loopThruNeighboursOfNode(
//   node: TNode,
//   index: number,
//   prev: TNode | null
// ) {
//   // tree[index] = node;
//   // tree[index].branch = node.neighbours;
//   // console.log(tree);
//   // node.visited = true;
//   paths[index] = [];
//   // paths[index].push(node.char);
//   console.log(paths);

//   node.neighbours.forEach((neighbour) => {
//     // const { col: ccol, row: crow, char: cchar } = node;
//     // const { col: ncol, row: nrow, char: nchar } = neighbour;
//     if (neighbour === prev || neighbour.visited) {
//       // debugger;
//       return;
//     }
//     if (neighbour.end) {
//       // finished = true;
//       // return path.push(neighbour);
//       debugger;
//     }
//     paths[index].push(neighbour.char);
//     return loopThruNeighboursOfNode(neighbour, ++index, node);
//   });
// }

// loopThruNeighboursOfNode(startNode, 0, null);
console.log(path.length - 1);
// loopThruNeighbours(data[S[0], S[1]])
Deno.exit();
//data.forEach((row) => console.log(row.split("")));
...
data.forEach((line, indexY) => {
  line.forEach((point, indexX) => {
    ...
    point.neighbours = setNeighbours(indexX, indexY, point.code);
    console.log(point, indexX, indexY, point.neighbours);

    const currentnode = map.get(point);
    if (currentnode) {
      currentnode.count++;
      currentnode.pos.push([indexX, indexY]);
    } else {
      map.set(point, {
        count: 1,
        pos: [[indexX, indexY]],
    ...
    // const count = (map.get(char) ? map.get(char).count : 0) + 1;
...
console.log(map, S, E, gridWidth, gridHeight);
// console.table(data);
